...
const STORAGE_KEY='kenguru-reviews-cache-v7';
const LEGACY_KEYS=['kenguru-reviews-cache-v6','kenguru-reviews-cache-v5','kenguru-reviews-cache-v4'];
...
function parseStored(raw){try{const data=JSON.parse(raw||'null');return data&&Array.isArray(data.reviews)&&data.reviews.length?{updatedAt:data.updatedAt||0,reviews:normalizeReviews(data.reviews)}:null}catch{return null}}
function readLocalCache(){let best=parseStored(localStorage.getItem(STORAGE_KEY));for(const key of LEGACY_KEYS){const item=parseStored(localStorage.getItem(key));if(item&&(!best||item.updatedAt>best.updatedAt))best=item}return best}
...
async function requestReviews(force=false,timeout=12000){
  ...
  try{
    ...
    if(!response.ok||!type.includes('application/json'))throw new Error('temporary');
    const data=await response.json();
    if(!Array.isArray(data.reviews)||!data.reviews.length)throw new Error('empty');
    return data;
  }finally{clearTimeout(timer)}
}

function applyData(data){const fresh=normalizeReviews(data?.reviews);if(!fresh.length)return false;const before=reviews.map(reviewKey).join('||');const after=fresh.map(reviewKey).join('||');saveLocalCache(data);if(before!==after){reviews=fresh;index=Math.min(index,reviews.length-1);render();startAuto()}statusEl.textContent='';return true}

async function refreshInBackground(){
  try{applyData(await requestReviews(true,15000))}catch{/* Всегда оставляем последнюю успешную копию на экране. */}
}

async function boot(){
  const local=readLocalCache();
  if(local?.reviews?.length){reviews=local.reviews;render();startAuto();statusEl.textContent=''}
  else statusEl.textContent='Загружаем отзывы…';

  // Быстрый запрос получает серверный кэш. Он не ждёт обновления Яндекса.
  try{applyData(await requestReviews(false,12000))}catch{
    // Не показываем посетителю техническую ошибку. Старые карточки остаются.
    if(!reviews.length)statusEl.textContent='';
  }

  // Новые отзывы подтягиваются незаметно после отрисовки старых.
  setTimeout(refreshInBackground,3000);
}

boot();
setInterval(refreshInBackground,30*60*1000);
